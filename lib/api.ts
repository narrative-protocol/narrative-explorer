...
export interface EventDetail {
  ...
  onChain: {
    solana: { signature: string; eventRecordPda: string; explorerUrl: string } | null
...
export async function getWorlds(page = 1, limit = 20): Promise<Paginated<WorldSummary>> {
  const data = await fetch(`${BASE_URL}/worlds?page=${page}&limit=${limit}`, { next: { revalidate: 60 } })
...
export async function getDeployments(page = 1, limit = 20): Promise<Paginated<DeploymentSummary>> {
  const data = await fetch(`${BASE_URL}/deployments?page=${page}&limit=${limit}`, { next: { revalidate: 60 } })
  const json = await data.json()
  return { data: json.data, pagination: json.pagination }
}

export async function getDeploymentDetail(id: number): Promise<DeploymentDetail> {
  return fetchAPI<DeploymentDetail>(`/deployments/${id}`)
}

export async function getDeploymentEntities(id: number, page = 1, limit = 20): Promise<Paginated<EntityInstance>> {
  const data = await fetch(`${BASE_URL}/deployments/${id}/entities?page=${page}&limit=${limit}`, { next: { revalidate: 30 } })
  const json = await data.json()
  return { data: json.data, pagination: json.pagination }
}

export async function getDeploymentHistory(id: number, limit = 20, cursor?: string) {
  const params: Record<string, string> = { limit: String(limit) }
  if (cursor) params.cursor = cursor
  return fetchAPI<EventHistoryItem[]>(`/deployments/${id}/history`, params)
}

export async function getGlobalEvents(page = 1, limit = 20): Promise<Paginated<EventHistoryItem>> {
  const data = await fetch(`${BASE_URL}/events?page=${page}&limit=${limit}`, { next: { revalidate: 30 } })
  const json = await data.json()
  return { data: json.data, pagination: json.pagination }
}
