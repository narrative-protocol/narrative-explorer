  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard
          label="Entities"
          value={data.stats.entityInstanceCount}
          icon={<Users className="h-4 w-4" />}
        />
        <StatCard
          label="Events Executed"
          value={data.stats.eventHistoryCount}
          icon={<Activity className="h-4 w-4" />}
        />
        <StatCard
          label="Event Bindings"
          value={data.eventBindings.length}
          icon={<Boxes className="h-4 w-4" />}
        />
        <StatCard
          label="Target Chain"
          value={data.targetChain}
                ...
                {data.eventBindings.map((binding) => (
                  <tr key={binding.id} className="hover:bg-secondary/30">
                    ...
                    <td className="px-4 py-3 font-mono text-muted-foreground">
                      v{binding.eventVersion.version}
                    </td>
                    <td className="px-4 py-3 text-right text-muted-foreground">
                      {new Date(
                        binding.eventVersion.publishedAt
                      ).toLocaleDateString()}
                    </td>
                  </tr>
                ))}
