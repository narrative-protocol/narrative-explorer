...
export function WorldsListContent({
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: "Home", href: "/" }, { label: "Worlds" }]} />

      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Worlds</h1>
          <p className="mt-1 text-sm text-muted-foreground">Public game worlds on the Narrative Protocol</p>
        </div>
        {result && (
          <span className="text-sm text-muted-foreground">{result.pagination.total} worlds</span>
        )}
      </div>
      ...
      {result && result.data.length === 0 && <EmptyState message="No public worlds found" />}
            ...
            {result.data.map((world) => (
              ...
              >
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-foreground group-hover:text-primary transition-colors">{world.name}</h3>
                </div>
                {world.description && (
                  <p className="mb-3 line-clamp-2 text-sm leading-relaxed text-muted-foreground">{world.description}</p>
                )}
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                  <span>{world.publicDeploymentCount} deployment{world.publicDeploymentCount !== 1 ? "s" : ""}</span>
